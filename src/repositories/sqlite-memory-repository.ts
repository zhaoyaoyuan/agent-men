...
export class SQLiteMemoryRepository {
  ...
  async insert(record: MemoryRecordLike): Promise<void> {
    ...
    `).run(
      ...
      fullRecord.user_id,
      fullRecord.memory_type,
      fullRecord.scope_type,
      fullRecord.scope_key,
      fullRecord.title,
      fullRecord.content,
      fullRecord.summary,
      fullRecord.status,
      ...
      fullRecord.strength,
      fullRecord.decay_score,
      fullRecord.importance_score,
      fullRecord.access_count,
      fullRecord.success_count,
      fullRecord.failure_count,
      fullRecord.last_accessed_at,
      fullRecord.last_verified_at,
      fullRecord.last_reinforced_at,
      fullRecord.expires_at,
      fullRecord.source_strategy,
      fullRecord.explanation_json,
      fullRecord.metadata_json,
  ...
  async findByProjectId(projectId: string): Promise<MemoryRecord[]> {
  ...
  async findByProjectIdAndType(projectId: string, memoryType: MemoryRecord['memory_type']): Promise<MemoryRecord[]> {
    const rows = this.db.prepare(`
      SELECT id, project_id, user_id, memory_type, scope_type, scope_key, title, content, summary,
             status, confidence, strength, decay_score, importance_score, access_count,
             success_count, failure_count, last_accessed_at, last_verified_at, last_reinforced_at,
      ...
      ORDER BY created_at DESC
    `).all(projectId, memoryType) as MemoryRecord[]
  ...
  async update(id: string, updates: Partial<Omit<MemoryRecord, 'id' | 'created_at'>>): Promise<void> {
