...
describe('createExtractMemoriesService', () => {
    ...
    it('should extract multiple memories with title and content from event content using LLM', async () => {
        ...
        expect(mockLLMClient.extractMemoriesFromEvent).toHaveBeenCalledWith(expect.stringContaining('I prefer dark mode'));
    ...
    it('should handle different memory types correctly', async () => {
        ...
        const mockLLMClient = {
            extractMemoriesFromEvent: vi.fn(async () => [
                { memoryType: 'fact', title: 'Fact 1', content: 'Content 1' },
                { memoryType: 'constraint', title: 'Constraint 1', content: 'Content 2' },
                { memoryType: 'preference', title: 'Preference 1', content: 'Content 3' },
                { memoryType: 'task_state', title: 'Task State', content: 'Task was completed' },
                { memoryType: 'experience', title: 'Experience', content: 'This approach worked well' },
            ]),
        };
        const service = createExtractMemoriesService({
            projectRepository: {
