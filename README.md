...
**工作流程：**
1. 自动检测当前项目
2. **自动创建项目** - 如果项目不存在，会自动调用 API 创建
3. 将内容作为 `decision` 事件摄入
4. 提示你复制 `eventId` 进行记忆提取
5. 提取完成后，记忆永久保存

### `/memory-os-auto` - 全自动工作流

全自动工作流，开始任务前自动召回，完成后提醒你摄入：

```
/memory-os-auto task=实现用户登录功能
```

## 可视化浏览面板

Memory OS 内置了一个可视化 web 面板，可以在浏览器中浏览查看所有项目和记忆：
...
- 下拉选择项目
- 关键词搜索记忆
- 按记忆类型过滤（Fact/Constraint/Preference/Task State/Experience）
- 响应式卡片布局展示
- 显示类型标签、标题、内容、confidence/strength/importance 分数条和更新时间
- 支持移动端适配
